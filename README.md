...
4. The value of ngModel won't be set unless it passes validation for the input field. For example: inputs of type email must have a value in the form of user@domain
...
1. AngularJS services are substitutable objects that are wired together using dependency injection (DI). You can use services to organize and share code across your app. AngularJS services are: 

2. Lazily instantiated – AngularJS only instantiates a service when an application component depends on it.

3. Singletons – Each component dependent on a service gets a reference to the single instance generated by the service factory.
